...
class InboxNotifier extends StateNotifier<Inbox> {
  InboxNotifier()
      : super(Inbox(
          isShowAllMode: true,
          todos: [
  ...
  void toggleShowAllMode() {
    state = state.copyWith(isShowAllMode: !state.isShowAllMode);
  }

  void toggleTodoCompleted(String todoId) {
    final todos = state.todos;
    final prev = todos.firstWhere((element) => element.id == todoId);
    final curr = prev.copyWith(isCompleted: !prev.isCompleted);

    state = state.copyWith(
      todos: todos
          .map((element) => element.id == todoId ? curr : element)
          .toList(),
    );
  }
}

final inboxProvider = StateNotifierProvider<InboxNotifier, Inbox>((ref) {
